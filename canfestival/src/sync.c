...
** @param id                                                                                       
**/  
void SyncAlarm(CO_Data* d, uint32_t id);
uint32_t OnCOB_ID_SyncUpdate(CO_Data* d, const indextable * unsused_indextable, 
	uint8_t unsused_bSubindex);
...
** @return                                                                                         
**/  
uint32_t OnCOB_ID_SyncUpdate(CO_Data* d, const indextable * unsused_indextable, uint8_t unsused_bSubindex)
{
	startSYNC(d);
	return 0;
}
...
** @return                                                                                         
**/  
uint8_t sendSYNCMessage(CO_Data* d)
{
  Message m;
  
  MSG_WAR(0x3001, "sendSYNC ", 0);
  
  m.cob_id = (uint16_t)LEndian16(*d->COB_ID_Sync);
  m.rtr = NOT_A_REQUEST;
  m.len = 0;
