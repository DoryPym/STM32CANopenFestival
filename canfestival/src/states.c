...
{
	uint16_t cob_id = LEndian16(m->cob_id);
	MSG_WAR(0x000,"after cob_id >>7 is ",(cob_id >> 7));
	MSG_WAR(0x000,"LEndian16(m->cob_id) is ",LEndian16(m->cob_id));
...
** @return                                                                                         
**/  
uint8_t setState(CO_Data* d, e_nodeState newState)
{
...
** @param nodeId                                                                                   
**/   
void setNodeId(CO_Data* d, uint8_t nodeId)
{
  uint16_t offset = d->firstIndex->SDO_SVR;
  ...
  if(offset){
    ...
    if((*(uint32_t*)d->objdict[offset].pSubindex[1].pObject == 0x600 + *d->bDeviceNodeId)||(*d->bDeviceNodeId==0xFF)){
      /* cob_id_client = 0x600 + nodeId; */
      *(uint32_t*)d->objdict[offset].pSubindex[1].pObject = 0x600 + nodeId;
    }
    /* Adjust COB-ID Server -> Client (tx) only id already set to default value or id not valid (id==0xFF)*/
    if((*(uint32_t*)d->objdict[offset].pSubindex[2].pObject == 0x580 + *d->bDeviceNodeId)||(*d->bDeviceNodeId==0xFF)){
      /* cob_id_server = 0x580 + nodeId; */
      *(uint32_t*)d->objdict[offset].pSubindex[2].pObject = 0x580 + nodeId;
    }
  }
  ...
  */
  {
    ...
    uint32_t cobID[] = {0x200, 0x300, 0x400, 0x500};
    if( offset ) while( (offset <= lastIndex) && (i < 4)) {
      if((*(uint32_t*)d->objdict[offset].pSubindex[1].pObject == cobID[i] + *d->bDeviceNodeId)||(*d->bDeviceNodeId==0xFF))
	      *(uint32_t*)d->objdict[offset].pSubindex[1].pObject = cobID[i] + nodeId;
      i ++;
      offset ++;
    }
  }
  ...
  {
    uint8_t i = 0;
    uint16_t offset = d->firstIndex->PDO_TRS;
    uint16_t lastIndex = d->lastIndex->PDO_TRS;
    uint32_t cobID[] = {0x180, 0x280, 0x380, 0x480};
    i = 0;
    if( offset ) while ((offset <= lastIndex) && (i < 4)) {
      if((*(uint32_t*)d->objdict[offset].pSubindex[1].pObject == cobID[i] + *d->bDeviceNodeId)||(*d->bDeviceNodeId==0xFF))
	      *(uint32_t*)d->objdict[offset].pSubindex[1].pObject = cobID[i] + nodeId;
