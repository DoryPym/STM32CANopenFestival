...
uint32_t OnNumberOfErrorsUpdate(CO_Data* d, const indextable * unsused_indextable, uint8_t unsused_bSubindex);
uint8_t sendEMCY(CO_Data* d, uint16_t errCode, uint8_t errRegister);
...
uint8_t sendEMCY(CO_Data* d, uint16_t errCode, uint8_t errRegister)
{
	...
	m.cob_id = (uint16_t)LEndian16(*(uint32_t*)d->error_cobid);
	m.rtr = NOT_A_REQUEST;
	m.len = 8;
 ...
 */
uint8_t EMCY_setError(CO_Data* d, uint16_t errCode, uint8_t errRegMask, uint16_t addInfo)
{
	uint8_t index;
	uint8_t errRegister_tmp;
	...
	*(d->error_first_element) = errCode | ((uint32_t)addInfo << 16);
 ...
 ** @return 1 if error, 0 if successful
 */
void EMCY_errorRecovered(CO_Data* d, uint16_t errCode)
{
	uint8_t index;
	uint8_t errRegister_tmp;
	uint8_t anyActiveError = 0;
...
void proceedEMCY(CO_Data* d, Message* m)
{
	...
	nodeID = LEndian16(m->cob_id) & 0x7F;
	errCode = m->data[0] | ((uint16_t)m->data[1] << 8);
	errReg = m->data[2];
	(*d->post_emcy)(d, nodeID, errCode, errReg);
}

void _post_emcy(CO_Data* d, uint8_t nodeID, uint16_t errCode, uint8_t errReg){}
