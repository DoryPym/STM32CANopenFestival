...
#define getLSSIdent(msg) (((uint32_t)msg->data[4] << 24) | ((uint32_t)msg->data[3] << 16) | (msg->data[2] << 8) | (msg->data[1]))
...
uint8_t sendMasterLSSMessage(CO_Data* d, uint8_t command,void *dat1,void *dat2);
	...
	if(d->lss_transfer.command==LSS_IDENT_FASTSCAN){
		if(d->lss_transfer.FastScan_SM==LSS_FS_RESET){
   			...
   			if(d->lss_transfer.LSSanswer!=0){
   				...
   				Mask=(uint32_t)((UNS64)Mask<<(d->lss_transfer.BitChecked+1));
...
** @return                                                                                         
**/  
uint8_t sendSlaveLSSMessage(CO_Data* d, uint8_t command,void *dat1,void *dat2)
{
  Message m;
  uint8_t i;
  ...
  case LSS_INQ_SERIAL_NUMBER: /* Inquire Identity Serial-Number */
	m.data[1]=(uint8_t)(*(uint32_t*)dat1 & 0xFF);
	m.data[2]=(uint8_t)(*(uint32_t*)dat1>>8 & 0xFF);
	m.data[3]=(uint8_t)(*(uint32_t*)dat1>>16 & 0xFF);
	m.data[4]=(uint8_t)(*(uint32_t*)dat1>>24 & 0xFF);
	break;
...
static uint8_t CO_TranslateBaudRate(char* optarg){
...
uint8_t sendMasterLSSMessage(CO_Data* d, uint8_t command,void *dat1,void *dat2)
{
  ...
  m.rtr = NOT_A_REQUEST;
  m.data[0]=command;
  m.cob_id=LEndian16(MLSS_ADRESS);
  ...
  case LSS_CONF_ACT_BIT_TIMING: /* Activate Bit Timing Parameters */
	...
	if(d->lss_transfer.baudRate!="none"){
		d->lss_transfer.switchDelay=(uint16_t)(*(uint32_t*)dat1 & 0xFFFF);
  ...
  case LSS_IDENT_FASTSCAN:
		...
		m.data[1]=(uint8_t)(d->lss_transfer.IDNumber & 0xFF);
		m.data[2]=(uint8_t)(d->lss_transfer.IDNumber>>8 & 0xFF);
		m.data[3]=(uint8_t)(d->lss_transfer.IDNumber>>16 & 0xFF);
		m.data[4]=(uint8_t)(d->lss_transfer.IDNumber>>24 & 0xFF);
...
{  
   ...
   //switch(msg_cs=m->data[0]){
	...
	case LSS_SM_SELECTIVE_SERIAL:
	{
		...
		if(_SpecificNodeInfo==*(uint32_t*)ptrTable->pSubindex[msg_cs-(LSS_SM_SELECTIVE_VENDOR-1)].pObject){
	...
	case LSS_IDENT_REMOTE_SERIAL_HIGH:
	{
      ...
      vendorId =  (uint32_t*) d->objdict[9].pSubindex[1].pObject;
		
		if(_SpecificNodeInfo == *(uint32_t*)ptrTable->pSubindex[1].pObject)
			 MSG_WAR(0x3D33, " ================= ", 0);
			
		/* Check if the data match the identity object. */
		switch(msg_cs){
		case LSS_IDENT_REMOTE_VENDOR:d->lss_transfer.addr_ident_match=(_SpecificNodeInfo == *(uint32_t*)ptrTable->pSubindex[1].pObject)? d->lss_transfer.addr_ident_match|0x01:0;	break;
		case LSS_IDENT_REMOTE_PRODUCT:d->lss_transfer.addr_ident_match=(_SpecificNodeInfo == *(uint32_t*)ptrTable->pSubindex[2].pObject)? d->lss_transfer.addr_ident_match|0x02:0;	break;
		case LSS_IDENT_REMOTE_REV_LOW:d->lss_transfer.addr_ident_match=(_SpecificNodeInfo <= *(uint32_t*)ptrTable->pSubindex[3].pObject)? d->lss_transfer.addr_ident_match|0x04:0; break;
		case LSS_IDENT_REMOTE_REV_HIGH:d->lss_transfer.addr_ident_match=(_SpecificNodeInfo >= *(uint32_t*)ptrTable->pSubindex[3].pObject)? d->lss_transfer.addr_ident_match|0x08:0;	break;
		case LSS_IDENT_REMOTE_SERIAL_LOW:d->lss_transfer.addr_ident_match=(_SpecificNodeInfo <= *(uint32_t*)ptrTable->pSubindex[4].pObject)? d->lss_transfer.addr_ident_match|0x10:0;	break;
		case LSS_IDENT_REMOTE_SERIAL_HIGH:d->lss_transfer.addr_ident_match=(_SpecificNodeInfo >= *(uint32_t*)ptrTable->pSubindex[4].pObject)? d->lss_transfer.addr_ident_match|0x20:0;	break;
		}
		/* If all the fields has been set.. */
	...
	if(d->lss_transfer.mode==LSS_CONFIGURATION_MODE)
	{
		...
		uint32_t errorCode;
  		...
  		ptrTable = (*d->scanIndexOD)(0x1018, &errorCode, &Callback);
  		_SpecificNodeInfo=*(uint32_t*)ptrTable->pSubindex[msg_cs-(LSS_INQ_VENDOR_ID-1)].pObject;
	...
	case LSS_IDENT_FASTSCAN:
	{
		...
		if(getLSSBitCheck(m)==128)d->lss_transfer.FastScan_SM=LSS_FS_RESET;
   		...
   		case LSS_FS_RESET:
   		{
			...
			d->lss_transfer.FastScan_SM=LSS_FS_PROCESSING;
  			...
  			ptrTable = (*d->scanIndexOD)(0x1018, &errorCode, &Callback);
  			d->lss_transfer.IDNumber=*(uint32_t*)ptrTable->pSubindex[d->lss_transfer.LSSPos+1].pObject;
		...
		case LSS_FS_PROCESSING:/*if(getLSSBitCheck(m)<32)*/
			...
			{
				uint32_t Mask=0xFFFFFFFF<<getLSSBitCheck(m);
...
void _lss_StoreConfiguration(uint8_t *error, uint8_t *spec_error){printf("_lss_StoreConfiguration\n");}
