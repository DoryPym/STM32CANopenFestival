...
** @return
**/
uint8_t masterSendNMTstateChange(CO_Data* d, uint8_t Node_ID, uint8_t cs)
{
  Message m;
...
** @return
**/
uint8_t masterSendNMTnodeguard(CO_Data* d, uint8_t nodeId)
{
  Message m;
  /* message configuration */
  uint16_t tmp = nodeId | (NODE_GUARD << 7); 
  m.cob_id = LEndian16(tmp);
  m.rtr = REQUEST;
  m.len = 0;
...
** @return
**/
uint8_t masterRequestNodeState(CO_Data* d, uint8_t nodeId)
{
