...
static uint8_t read_consise_dcf_next_entry(CO_Data* d, uint8_t nodeId);
static uint8_t write_consise_dcf_next_entry(CO_Data* d, uint8_t nodeId);
uint8_t init_consise_dcf(CO_Data* d,uint8_t nodeId);
...
** @param nodeId
*/
uint8_t check_and_start_node(CO_Data* d, uint8_t nodeId)
{   
...
uint8_t init_consise_dcf(CO_Data* d,uint8_t nodeId)
{
    ...
    if(! d->dcf_odentry->pSubindex[nodeId].size) goto DCF_finish;
		{
    	uint8_t* dcf = (uint8_t*)d->dcf_odentry->pSubindex[nodeId].pObject;
...
uint8_t get_next_DCF_data(CO_Data* d, dcf_entry_t *dcf_entry, uint8_t nodeId)
{
  uint8_t* dcfend;
  uint32_t nb_entries;
  uint32_t szData;
  ...
  szData = d->dcf_odentry->pSubindex[nodeId].size;
	//uint8_t* dcf = (uint8_t*)d->dcf_odentry->pSubindex[nodeId].pObject;
	nb_entries = LEndian32(*((uint32_t*)dcf));
  dcfend = dcf + szData;
  if((uint8_t*)d->dcf_cursor + 7 < (uint8_t*)dcfend && d->dcf_entries_count < nb_entries){
...
static uint8_t write_consise_dcf_next_entry(CO_Data* d, uint8_t nodeId)
{
    ...
    Ret = writeNetworkDictCallBackAI(d, /* CO_Data* d*/
                    ...
                    dcf_entry.Index, /* uint16_t index*/
                    dcf_entry.Subindex, /* uint8_t subindex*/
                    (uint8_t)dcf_entry.Size, /* uint8_t count*/
                    0, /* uint8_t dataType*/
...
{
    uint8_t Ret;
    uint32_t data=0x65766173;
    Ret = writeNetworkDictCallBackAI(d, /* CO_Data* d*/
                    nodeId, /* uint8_t nodeId*/
                    0x1010, /* uint16_t index*/
                    1, /* uint8_t subindex*/
                    4, /* uint8_t count*/
                    0, /* uint8_t dataType*/
                    (void *)&data,/* void *data*/
