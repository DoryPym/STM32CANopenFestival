...
uint8_t GetSDOClientFromNodeId( CO_Data* d, uint8_t nodeId );
 ...
 ** @return
 **/
INLINE uint8_t _writeNetworkDict (CO_Data* d, uint8_t nodeId, uint16_t index,
 ...
 ** @return
 **/
INLINE uint8_t _readNetworkDict (CO_Data* d, uint8_t nodeId, uint16_t index, uint8_t subIndex,
		uint8_t dataType, SDOCallback_t Callback, uint8_t useBlockMode);
...
*/
#define getSDOindex(byte1, byte2) (((uint16_t)byte2 << 8) | ((uint16_t)byte1))
...
void SDOTimeoutAlarm(CO_Data* d, uint32_t id)
{
	...
	nodeId = *((uint32_t*) d->objdict[offset+d->transfers[id].CliServNbr].pSubindex[3].pObject);
...
{
    uint32_t  size = SDO_MAX_LENGTH_TRANSFERT;
	uint8_t  dataType;
	uint32_t errorCode;
    ...
    if (errorCode == SDOABT_OUT_OF_MEMORY) {
        ...
        * 'size' now contains the real size of the requested object.             */
        if (size <= SDO_DYNAMIC_BUFFER_ALLOCATION_SIZE) {
            d->transfers[line].dynamicData = (uint8_t *) malloc(size * sizeof(uint8_t));
...
#ifdef SDO_DYNAMIC_BUFFER_ALLOCATION
	{
		uint8_t* lineData = d->transfers[line].data;
		if ((d->transfers[line].offset + nbBytes) > SDO_MAX_LENGTH_TRANSFERT) {
			if (d->transfers[line].dynamicData == NULL) {
				d->transfers[line].dynamicData = (uint8_t*) malloc(SDO_DYNAMIC_BUFFER_ALLOCATION_SIZE);
			...
			{
				uint8_t* newDynamicBuffer = (uint8_t*) realloc(d->transfers[line].dynamicData, d->transfers[line].dynamicDataSize + SDO_DYNAMIC_BUFFER_ALLOCATION_SIZE);
 ...
 ** @return
 **/
uint8_t failedSDO (CO_Data* d, uint8_t CliServNbr, uint8_t whoami, uint16_t index,
 ...
 ** @return
 **/
uint8_t closeSDOtransfer (CO_Data* d, uint8_t nodeId, uint8_t whoami)
{
	uint8_t err;
	uint8_t line;
    uint8_t CliNbr;
...
uint8_t sendSDO (CO_Data* d, uint8_t whoami, uint8_t CliServNbr, uint8_t *pData)
{
	...
	if ( whoami == SDO_SERVER )	{
		...
		m.cob_id = *((uint32_t*) d->objdict[offset+CliServNbr].pSubindex[2].pObject); //zxb add this
		...
		m.cob_id = *((uint32_t*) d->objdict[offset+CliServNbr].pSubindex[1].pObject);
...
uint8_t sendSDOabort (CO_Data* d, uint8_t whoami, uint8_t CliServNbr, uint16_t index, uint8_t subIndex, uint32_t abortCode)
{
	...
	data[4] = (uint8_t)(abortCode & 0xFF);
	data[5] = (uint8_t)((abortCode >> 8) & 0xFF);
	data[6] = (uint8_t)((abortCode >> 16) & 0xFF);
	data[7] = (uint8_t)((abortCode >> 24) & 0xFF);
...
uint8_t proceedSDO (CO_Data* d, Message *m)
{
	...
	uint8_t cs;
	uint8_t line;
	uint32_t nbBytes; 		/* received or to be transmited. */
	uint8_t nodeId = 0;  	/* The node Id of the server if client otherwise unused */
	uint8_t CliServNbr;
	uint8_t whoami = SDO_UNKNOWN;  /* SDO_SERVER or SDO_CLIENT.*/
	uint32_t errorCode; /* while reading or writing in the local object dictionary.*/
	uint8_t data[8];    /* data for SDO to transmit */
	uint16_t index;
	uint8_t subIndex;
	uint32_t abortCode;
	uint32_t i;
	uint8_t	j;
	uint32_t *pCobId = NULL;
	uint32_t *pCobId_test = NULL;
	uint16_t offset;
	uint16_t lastIndex;
	uint8_t SubCommand;	/* Block transfert only */
  uint8_t SeqNo;         /* Sequence number in block transfert */
  uint8_t AckSeq;        /* Sequence number of last segment that was received successfully */
	uint8_t NbBytesNoData; /* Number of bytes that do not contain data in last segment of block transfert */ 
	...
	if(offset) while (offset <= lastIndex) {
		...
		pCobId = (uint32_t*) d->objdict[offset].pSubindex[1].pObject;
		pCobId_test = (uint32_t*) d->objdict[offset].pSubindex[2].pObject;
		...
		MSG_WAR(0x3A60, "LEndian16(m->cob_id)  is ",LEndian16(m->cob_id));
		MSG_WAR(0x3A60, "m->cob_id  is ",m->cob_id);
		if ( *pCobId == LEndian16(m->cob_id) ) {
			whoami = SDO_SERVER;
	...
	if (whoami == SDO_UNKNOWN) {
		...
		if(!offset) while (offset <= lastIndex) {//zxb change this
			...
			/* Looking for the cobid received. */
			pCobId = (uint32_t*) d->objdict[offset].pSubindex[2].pObject;
			MSG_WAR(0x0000,"pCobId is ",*pCobId);
			MSG_WAR(0x0000,"m->cob_id is ",m->cob_id);
			MSG_WAR(0x0000,"LEndian16(m->cob_id) is ",LEndian16(m->cob_id));
			if (*pCobId == LEndian16(m->cob_id) ) {
				whoami = SDO_CLIENT;
				...
				nodeId = *((uint8_t*) d->objdict[offset].pSubindex[3].pObject);
	...
	switch (cs) {
		...
		case 1:
			...
			if (whoami == SDO_SERVER) {
				...
				if (getSDOe(m->data[0])) { /* If SDO expedited */
					...
					if (getSDOs(m->data[0])) {
						nbBytes = (m->data[4]) + ((uint32_t)(m->data[5])<<8) + ((uint32_t)(m->data[6])<<16) + ((uint32_t)(m->data[7])<<24);
				...
				if (nbBytes > 7) {
					...
					d->transfers[line].toggle = ! d->transfers[line].toggle & 1;
					data[0] = (uint8_t)((d->transfers[line].toggle << 4) | ((7 - nbBytes) << 1) | 1);
		...
		case 4:
			abortCode =
				(uint32_t)m->data[4] |
				((uint32_t)m->data[5] << 8) |
				((uint32_t)m->data[6] << 16) |
				((uint32_t)m->data[7] << 24);
			/* Received SDO abort. */
			if (whoami == SDO_SERVER) {
				if (!err) {
					resetSDOline( d, line );
			...
			if (whoami == SDO_SERVER) { /* Server block download */
				if (err) {
					...
					if ((m->data[0]) & 2)	/* if data set size is indicated */
                    	d->transfers[line].objsize = (uint32_t)m->data[4] + (uint32_t)m->data[5]*256 + (uint32_t)m->data[6]*256*256 + (uint32_t)m->data[7]*256*256*256;
...
uint8_t GetSDOClientFromNodeId( CO_Data* d, uint8_t nodeId )
{
	...
	while (offset <= lastIndex) {
		...
		nodeIdServer = *((uint8_t*) d->objdict[offset].pSubindex[3].pObject);
		...
		uint8_t subIndex, uint32_t count, uint8_t dataType, void *data, SDOCallback_t Callback, uint8_t endianize, uint8_t useBlockMode)
{
...
#ifdef SDO_DYNAMIC_BUFFER_ALLOCATION
	{
		...
		{
			d->transfers[line].dynamicData = (uint8_t*) malloc(count);
		...
		uint8_t subIndex, uint32_t count, uint8_t dataType, void *data, SDOCallback_t Callback, uint8_t endianize, uint8_t useBlockMode)
{
	...
	if(ret == 0xFE)
	{
		...
		while (offset <= lastIndex)
		{
			...
			if(nodeIdServer == 0)
			{
				*(uint32_t*)d->objdict[offset].pSubindex[1].pObject = (uint32_t)(0x600 + nodeId);
				*(uint32_t*)d->objdict[offset].pSubindex[2].pObject = (uint32_t)(0x580 + nodeId);
				*(uint8_t*) d->objdict[offset].pSubindex[3].pObject = nodeId;
 ...
 ** @return
 **/
INLINE uint8_t _readNetworkDict (CO_Data* d, uint8_t nodeId, uint16_t index, uint8_t subIndex, uint8_t dataType, SDOCallback_t Callback, uint8_t useBlockMode)
{
	uint8_t err;
	uint8_t i;
	uint8_t CliNbr;
 ...
 **/
uint8_t getReadResultNetworkDict (CO_Data* d, uint8_t nodeId, void* data, uint32_t *size,
		uint32_t * abortCode)
{
	uint32_t i;
	uint8_t err;
