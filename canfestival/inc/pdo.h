...
void _RxPDO_EventTimers_Handler(CO_Data *d, uint32_t pdoNum);
...
struct struct_s_PDO_status {
  uint8_t transmit_type_parameter;
 ...
 */
uint8_t sendPDOrequest( CO_Data* d, uint16_t RPDOIndex );
 ...
 */
uint8_t proceedPDO (CO_Data* d, Message *m);
 ...
 * @param *d Pointer on a CAN object data structure
 */
uint8_t sendPDOevent (CO_Data* d);
uint8_t sendOnePDOevent (CO_Data* d, uint8_t pdoNum);
 ...
 * @param isSyncEvent
 */
uint8_t _sendPDOevent(CO_Data* d, uint8_t isSyncEvent);
 ...
 */
void PDOEventTimerAlarm(CO_Data* d, uint32_t pdoNum);
 ...
 * @param pdoNum The PDO number
 */
void PDOInhibitTimerAlarm(CO_Data* d, uint32_t pdoNum);

/* copy bit per bit in little endian */
void CopyBits(uint8_t NbBits, uint8_t* SrcByteIndex, uint8_t SrcBitIndex, uint8_t SrcBigEndian, uint8_t* DestByteIndex, uint8_t DestBitIndex, uint8_t DestBigEndian);
#endif
