...
struct struct_CO_Data {
	/* Object dictionary */
	uint8_t *bDeviceNodeId; //node id
	const indextable *objdict;
	s_PDO_status *PDO_status;
	TIMER_HANDLE *RxPDO_EventTimers;
	void (*RxPDO_EventTimers_Handler)(CO_Data*, uint32_t);
	const quick_index *firstIndex;
	const quick_index *lastIndex;
	const uint16_t *ObjdictSize;
	const uint8_t *iam_a_slave;
	...
	/* NMT-heartbeat */
	uint8_t *ConsumerHeartbeatCount;
	uint32_t *ConsumerHeartbeatEntries;
	...
	uint8_t error_history_size;
	uint8_t* error_number;
	uint32_t* error_first_element;
	uint8_t* error_register;
    uint32_t* error_cobid;
