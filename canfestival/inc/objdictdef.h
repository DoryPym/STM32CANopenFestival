...
{
    uint8_t                    bAccessType;
    uint8_t                    bDataType; /* Defines of what datatype the entry is */
    uint32_t                   size;      /* The size (in Byte) of the variable */
...
typedef struct td_indextable
{
    ...
    uint8_t   bSubCount;   /* the count of valid entries for this subindex
...
typedef struct s_quick_index{
	...
	uint16_t SDO_CLT;
	uint16_t PDO_RCV;
	uint16_t PDO_RCV_MAP;
	uint16_t PDO_TRS;
	uint16_t PDO_TRS_MAP;
}quick_index;


/*typedef struct struct_CO_Data CO_Data; */
typedef uint32_t (*ODCallback_t)(CO_Data* d, const indextable *, uint8_t bSubindex);
typedef const indextable * (*scanIndexOD_t)(uint16_t wIndex, uint32_t * errorCode, ODCallback_t **Callback);

/************************** MACROS *********************************/

/* CANopen usefull helpers */
#define GET_NODE_ID(m)         (LEndian16(m.cob_id) & 0x7f)
#define GET_FUNCTION_CODE(m)   (LEndian16(m.cob_id) >> 7)
