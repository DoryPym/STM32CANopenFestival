...
/*****************************************************************************/
void TestSlave_heartbeatError(CO_Data* d, uint8_t heartbeatID)
{
	//eprintf("TestSlave_heartbeatError %d\n", heartbeatID);
	MSG_WAR(0x0000, "TestSlave_heartbeatError heartbeatID is",heartbeatID);
	//eprintf("TestSlave_heartbeatError %d\n", heartbeatID);
}

void TestSlave_initialisation(CO_Data* d)
{
	MSG_WAR(0x0000, "TestSlave_initialisation .........",0x0);
...
{
	MSG_WAR(0x0000, "TestSlave_preOperational .........",0x0);
...
{
	MSG_WAR(0x0000, "TestSlave_operational .........",0x0);
...
{
	MSG_WAR(0x0000, "TestSlave_stopped .........",0x0);
...
void TestSlave_post_sync(CO_Data* d)
{
	 ...
	 MSG_WAR(0x0000, "TestSlave_post_sync .........",0x0);
   // eprintf("TestSlave_post_sync\n");
      ...
      printf("Slave: %d %d %d %d %d %d %d %d %d %x %x %d %d \n",
      	SlaveMap1, 
      	SlaveMap2, 
      	SlaveMap3, 
      	SlaveMap4, 
...
{
  SlaveMap13 += 1;
  MSG_WAR(0x0000, "TestSlave_post_TPDO ......... SlaveMap13 ",SlaveMap13);
...
void TestSlave_storeODSubIndex(CO_Data* d, uint16_t wIndex, uint8_t bSubindex)
{
	 ...
	 * 
	 * */
	// MSG_WAR(0x0000, "TestSlave_storeODSubIndex wIndex:",wIndex);
	 //MSG_WAR(0x0000, "TestSlave_storeODSubIndex bSubindex:",bSubindex);
	 
//	eprintf("TestSlave_storeODSubIndex : %4.4x %2.2x\n", wIndex,  bSubindex);	
	printf("%s(%d) TestSlave_storeODSubIndex : %4.4x %2.2x\n", __FILE__,__LINE__,wIndex,  bSubindex);
}
...
void TestSlave_post_emcy(CO_Data* d, uint8_t nodeID, uint16_t errCode, uint8_t errReg)
{
	...
	printf("%s(%d) Slave received EMCY message. Node: %2.2x  ErrorCode: %4.4x  ErrorRegister: %2.2x\n", __FILE__,__LINE__,nodeID, errCode, errReg);
